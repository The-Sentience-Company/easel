...
The board is orchestrator-owned: one writer edits the data file and republishes; executors never touch it. Run each drafted entry through `queue-lint` (installed next to the `easel` CLI) before filing — a context-free cheap model flags shorthand the reader could not decode.
