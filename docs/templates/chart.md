...
The column runs 600–1400 units, defaulting to 1040. Two `sm` (960 plus the gutter) fit at the default width; two `md` (1296) fit only once the reader widens the page toward 1400; two `lg` never fit. Write for the default and let the wide case be a bonus — or use a chart row when the pairing must hold at any width.
