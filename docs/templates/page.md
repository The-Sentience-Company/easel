...
Where a single removed line is followed by added lines, the removed line and the **first** added line get word-level emphasis — the words that actually changed take a deeper tint of the same hue — and any further added lines are treated as wholly new. Punctuation and HTML entities are their own tokens, so a sentence extended past its full stop (`this is words` → `this is words. more words`) marks the addition alone rather than dragging the mark back over text that did not change. The two lines are aligned by longest common subsequence, so several edits on one line mark as several spans — `timeout 30 retries 3` → `timeout 60 retries 5` tints the two numbers, not everything between them. A run of several removed lines has no unambiguous pairing, so it gets the line tint only. Authoring rule: keep each amended line as its own `-`/`+` pair (unchanged lines as context) and the word tint shows the true delta; pasting a multi-line block as one removed run loses it.
