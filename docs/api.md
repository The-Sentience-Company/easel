...
Three publish-time `preRender` steps run over that html in a fixed order, each claiming its own `<pre>` class and leaving the others alone: `render/mermaid.js` (`pre.mermaid` → baked SVG), `render/diff.js` (`pre.sd-diff` → per-line markup), and `render/chart.js` (`pre.sd-chart` → a themed inline SVG chart). None of them throws; a block that will not render degrades to an error block and the document still publishes.
...
| Route | Body | Response |
|---|---|---|
| `POST /api/open` | `{"file?": "/abs.html", "template?": "review\|eval\|rulings\|page", "data?": "/abs.json", "title?": "…"}` | `{"key", "url", "created": true}`. **Idempotent**: an already-open board for the same `file` (or same `template`+`data` path) returns the existing session with `created: false`. Opening auto-publishes round 1 from current content. The daemon watches the source file(s); changes become WIP. `file` must be `.html`/`.htm` and `data` must be `.json` — anything else is 400 (other formats would store un-diffable, un-annotatable rounds). Without `title` a template board takes the `title` from its data, so the index names it something rather than its key; `publish` fills a still-blank one the same way, and never renames a board that already has one. |
| `POST /api/b/:key/publish` | `{"note?": "round 2: …", "agent?": "id", "amend?": false}` | `{"round": 2, "listenerDropped": false, "diff": {"added": [], "removed": [], "modified": [], "moved": []}, "audit": {"findings": […]}}`. Re-reads the source, renders, assigns `sid`s, diffs vs previous round, runs the advisory audit (never blocks), broadcasts `round`. With `agent` (the CLI sends the same id `await` uses), the publisher's own parked waiter resolves `{dropped: true}` synchronously — the exit lands inside the publish turn, never as a wake after the agent stops; other agents' waiters are untouched. |
| ↳ with `amend: true` | | Rewrites the **current** round instead of adding one; answers `amended: true` with the same `round`. It exists because the audit and reader checks only run once the round is written, so without it every correction of a finding costs the reader a round pill and splits the diff marks across two rounds. The diff is recomputed against the round *before* the one being replaced, so the marks stay whole. Refused `409` when that round already carries any feedback (it would rewrite what the reader replied to), when the board has no round yet, or when the round is older than 10 minutes (`EASEL_AMEND_WINDOW_MS`) — past that, assume it has been read. Amend never triggers auto-open. |
...
easel publish <key> [--note] [--amend]     → POST /api/b/:key/publish
...
- **Elements**: ordinary document markup (headings, lists, tables, `figure`, `details`, inline formatting, `a`, `img`, the widget `button`) plus the SVG set mermaid and the chart renderer emit. Everything else is unwrapped — the element and its attributes are dropped, its text-bearing children kept — so a hand-authored `page` board never silently loses a section. `<script>`, `<template>`, and any HTML-context `<style>` are dropped whole. `iframe`/`object`/`embed`/`base`/`meta`/`form`/`math` are simply not on the list.
...
| `sf-prior-answered` | `[data-widget]` node | The reader answered this widget on a prior round; the matching `[data-option]` carries `aria-pressed="true"` and renders pressed. Buttons stay live — a click records a new answer for the current round. |
