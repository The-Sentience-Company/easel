...
# The skill is a symlink into the checkout, never a copy — the running daemon
# and the skill's template docs stay in lockstep with no sync step.
SKILL_SRC="$ROOT/skills/easel"
SKILL_LINK="$HOME/.claude/skills/easel"

# Owned = a symlink into any easel checkout's skills dir, so a relocated
# checkout converges. A real dir or foreign link belongs to someone else.
owns_skill_link() {
  [ -L "$SKILL_LINK" ] || return 1
  case "$(readlink "$SKILL_LINK")" in */skills/easel) return 0 ;; *) return 1 ;; esac
}

install_skill_link() {
  mkdir -p "$(dirname "$SKILL_LINK")"
  if { [ -e "$SKILL_LINK" ] || [ -L "$SKILL_LINK" ]; } && ! owns_skill_link; then
    say "left $SKILL_LINK alone — not a link into an easel checkout"
    return 0
  fi
  ln -sfn "$SKILL_SRC" "$SKILL_LINK"
  say "skill: $SKILL_LINK -> $SKILL_SRC"
}
